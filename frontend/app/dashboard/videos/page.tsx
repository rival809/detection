  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-foreground">Daftar Video</h1>
          ...
          <table className="w-full text-sm min-w-[520px]">
            <thead>
              <tr className="border-b border-border">
                <th className="px-4 md:px-5 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Nama File</th>
                <th className="px-4 md:px-5 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                <th className="px-4 md:px-5 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Plat</th>
                <th className="px-4 md:px-5 py-3.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">Upload</th>
                <th className="px-4 md:px-5 py-3.5 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aksi</th>
              </tr>
            </thead>
            <tbody>
              {videos.length === 0 && (
                ...
                return (
                  <tr key={v.id} className="border-b border-border last:border-0 hover:bg-accent/40 transition-colors">
                    <td className="px-4 md:px-5 py-4">
                      <span className="font-medium text-foreground truncate max-w-[140px] sm:max-w-xs block">{v.original_filename}</span>
                    </td>
                    <td className="px-4 md:px-5 py-4">
                    ...
                    <td className="px-4 md:px-5 py-4 text-foreground font-semibold">{v.total_plates ?? "—"}</td>
                    <td className="px-4 md:px-5 py-4 text-muted-foreground text-xs hidden sm:table-cell">{new Date(v.uploaded_at).toLocaleString("id-ID")}</td>
                    <td className="px-4 md:px-5 py-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs text-muted-foreground">
          Hal. {page}/{totalPages} &middot; {total} video
        </p>
        <div className="flex items-center gap-1">
          <button
