      ...
      <div className="p-6 space-y-5 max-w-6xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h1 className="text-xl font-bold text-foreground truncate">{video.original_filename}</h1>
            <div className="flex items-center gap-3 mt-1.5 text-sm">
              <span className={`font-semibold ${statusStyle.color}`}>{statusStyle.label}</span>
              <span className="text-muted-foreground">·</span>
              <span className="text-muted-foreground">{video.total_plates} plat ditemukan</span>
              <span className="text-muted-foreground">·</span>
              <span className="text-muted-foreground text-xs">{new Date(video.uploaded_at).toLocaleString("id-ID")}</span>
            </div>
          </div>
          <button
            onClick={exportCSV}
            className="shrink-0 inline-flex items-center gap-2 px-4 py-2 border border-border text-sm font-medium rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
          >
        ...
        {isProcessing && (
          <div className="bg-card border border-border rounded-xl p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-foreground font-medium">{progress?.message ?? "Memulai pemrosesan..."}</span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Foto</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Nomor Plat</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Confidence</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status Pajak</th>
                <th className="px-5 py-3 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aksi</th>
              </tr>
            </thead>
            <tbody>
              {detections.length === 0 && (
              ...
              {detections.map((d) => {
                ...
                return (
                  <tr key={d.id} className="border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                      ...
                      {d.image_crop_url ? (
                        ...
                        >
                          <img
                            ...
                            alt={d.plate_number}
                            className="h-14 w-auto object-cover group-hover:opacity-80 transition-opacity"
                          />
                    ...
                    {/* Plat */}
                    <td className="px-5 py-3">
                      <span className="font-mono font-bold text-base text-foreground tracking-widest">{d.plate_number}</span>
                    </td>
                    ...
                    {/* Status pajak */}
                    <td className="px-5 py-3">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${tax.bg} ${tax.text}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${tax.dot}`} />
                        {tax.label}
                      </div>
                    </td>
                      ...
                      <div className="flex items-center gap-1.5 justify-end">
                        {(
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg border border-border text-muted-foreground hover:text-foreground hover:bg-accent disabled:opacity-50 transition-colors"
                          >
                            ...
                            {recheckingId === d.id ? "Mengecek..." : "Re-check"}
                          </button>
                        )}
                        ...
                        {confirmDeleteId === d.id ? (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-destructive text-white hover:bg-destructive/80 transition-colors disabled:opacity-50"
                            >
                              {deletingId === d.id ? "..." : "Hapus"}
                            </button>
                            <button
                              onClick={() => setConfirmDeleteId(null)}
                              className="px-2.5 py-1.5 text-xs font-medium rounded-lg border border-border text-muted-foreground hover:bg-accent transition-colors"
                            >
                              Batal
                            </button>
                          </div>
